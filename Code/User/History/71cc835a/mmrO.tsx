...
const RegisterForm = () => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		try {
			...
			if (res.ok) {
				setToastMessage('Registration successful!'); // Set pesan toast
				setToastError(false); // Status success
				form.reset();
				setTimeout(() => push('/auth/login'), 2000); // Redirect ke halaman login
			} else {
				const errorData = await res.json();
				setToastMessage(errorData.message || 'Registration failed. Please try again.');
				setToastError(true); // Status error
			}
		} catch (error) {
