...
import { toast } from 'sonner'; // Untuk menampilkan feedback ke pengguna
...
const RegisterForm = () => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		try {
			...
			if (res.ok) {
				toast.success('Registration successful!'); // Feedback sukses
				form.reset();
				push('/auth/login');
			} else {
				const errorData = await res.json();
				toast.error(errorData.message || 'Registration failed. Please try again.'); // Feedback error
			}
		} catch (error) {
			console.error('Error during sign-up:', error);
			toast.error('An unexpected error occurred. Please try again.'); // Feedback error
		} finally {
