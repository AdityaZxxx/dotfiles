...
const authOptions: NextAuthOptions = {
	...
	secret: process.env.NEXT_PUBLIC_NEXTAUTH_SECRET, // Pastikan sudah didefinisikan di .env
				...
				if (!snapshot.empty) {
					...
					if (isPasswordValid) {
						return {
							...
							name: userData.name,
							role: userData.role || 'guest', // Pastikan role ada
						};
					} else {
						console.log('Password tidak valid untuk:', email); // Debugging password
						throw new Error('Invalid credentials');
					}
				} else {
					console.log('Pengguna tidak ditemukan untuk:', email); // Debugging pengguna tidak ditemukan
