...
const LoginForm = () => {
	...
	const callbackUrl = searchParams.get('callbackUrl') || process.env.NEXTAUTH_URL; // Pastikan callbackUrl default adalah '/'.
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		try {
			...
			console.log('Response from signIn:', res); // Debugging response dari signIn
			...
			if (res?.error) {
				...
				// Check if callbackUrl is defined and is a valid string before calling push
				if (callbackUrl && typeof callbackUrl === 'string') {
					push(callbackUrl); // Proceed if callbackUrl is valid
				} else {
					push('/'); // Fallback to home page if callbackUrl is invalid
				}
			}
		} catch (error) {
